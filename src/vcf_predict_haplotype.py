...
class ConsistentModelPool(nn.Module):
    """
    Ensemble of consistent models using torch.func API.
    """

    def __init__(self, path_list):
        super().__init__()

        models = [load_model(p) for p in path_list]
        self._template_model = models[0]
        self.params, self.buffers = stack_module_state(models)

        def fmodel(params, buffers, data):
            return functional_call(self._template_model, (params, buffers), (data,))
        self.fmodel = fmodel
...
def main(args):
    ...
    vcf = boda.data.VCF(
        ...
        max_allele_size=200, max_indel_size=200,
    )
    ...
    if args.use_simple_padding:
        vcf_data = boda.data.VcfDataset_SimplePadding(
            vcf.vcf, fasta_data.fasta, WINDOW_SIZE, 
            RELATIVE_START, RELATIVE_END, window_alignment=args.window_alignment,
            window_symmetric=args.window_symmetric, step_size=args.step_size, 
            left_flank='', right_flank='',
            use_contigs=args.use_contigs,
        )
    else:
    ...
    #######################################
    ## SEQUENCE EXTRACTION FOR VALIDATION ##
    #######################################
    if args.extract_original_sequences:
        from boda.data.fasta_datamodule import save_original_sequences
    ...
    if args.extract_center_sequences:
        from boda.data.fasta_datamodule import save_center_sequences
        if not args.use_simple_padding:
            raise ValueError("--extract_center_sequences can only be used with --use_simple_padding True")
        if args.center_sequence_output_file:
            center_seq_output_path = args.center_sequence_output_file
        else:
            base_path = args.output.rsplit('.', 1)[0]
            center_seq_output_path = f"{base_path}_center_sequences.tsv"
        save_center_sequences(
            vcf_data,
            center_seq_output_path,
            sample_indices=range(len(vcf_data))
        )

    if args.extract_flank_sequences:
        from boda.data.fasta_datamodule import save_flank_sequences
        if not args.use_simple_padding:
            raise ValueError("--extract_flank_sequences can only be used with --use_simple_padding True")
        if args.flank_sequence_output_file:
            flank_seq_output_path = args.flank_sequence_output_file
        else:
            base_path = args.output.rsplit('.', 1)[0]
            flank_seq_output_path = f"{base_path}_flank_sequences.tsv"
        save_flank_sequences(
            vcf_data,
            flank_seq_output_path,
            sample_indices=range(len(vcf_data))
        )

    if args.extract_full_sequences:
        from boda.data.fasta_datamodule import save_full_sequences
        if not args.use_simple_padding:
            raise ValueError("--extract_full_sequences can only be used with --use_simple_padding True")
        if args.full_sequence_output_file:
            full_seq_output_path = args.full_sequence_output_file
        else:
            base_path = args.output.rsplit('.', 1)[0]
            full_seq_output_path = f"{base_path}_full_sequences.tsv"
        save_full_sequences(
            vcf_data,
            full_seq_output_path,
            sample_indices=range(len(vcf_data))
        )
    ###########################
    ...
    ref_preds, alt_preds, skew_preds = [], [], []
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm.tqdm(vcf_loader)):
            ...
            if ref_allele.nelement() == 0 or alt_allele.nelement() == 0:
                print(f"Skipping batch {i} due to data processing error.")
                batch_size = len(ref_allele)
                ref_preds.append(torch.zeros((batch_size, len(args.feature_ids))))
                alt_preds.append(torch.zeros((batch_size, len(args.feature_ids))))
                skew_preds.append(torch.zeros((batch_size, len(args.feature_ids))))
            ...
            if not args.raw_predictions:
                if args.strand_reduction == 'gather':
                    strand_index = getattr(gatherings, args.strand_gathering)(all_preds[args.gather_source], dim=1)
                    strand_kwargs= {'index': strand_index}
                else:
                    strand_kwargs= {}
                proc_preds = {}
                proc_preds['ref'] = getattr(reductions, args.strand_reduction)(all_preds['ref'], dim=1, **strand_kwargs)
                proc_preds['alt'] = getattr(reductions, args.strand_reduction)(all_preds['alt'], dim=1, **strand_kwargs)
                proc_preds['skew']= getattr(reductions, args.strand_reduction)(all_preds['skew'], dim=1, **strand_kwargs)
                if args.window_reduction == 'gather':
                    if args.activity_filter is not None:
                        try:
                            indexing_tensor = getattr(activity_filtering, args.window_gathering)(proc_preds, args.gather_source, args.activity_filter, args.epsilon)
                    ...
                    window_index = getattr(gatherings, args.window_gathering)(indexing_tensor, dim=1)
                    window_kwargs = {'index': window_index}
                else:
                    window_kwargs = {}
                proc_preds['ref'] = getattr(reductions, args.window_reduction)(proc_preds['ref'], dim=1, **window_kwargs)
                proc_preds['alt'] = getattr(reductions, args.window_reduction)(proc_preds['alt'], dim=1, **window_kwargs)
                proc_preds['skew']= getattr(reductions, args.window_reduction)(proc_preds['skew'], dim=1, **window_kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--window_alignment', type=str, choices=('left','right','middle'), default='left', help='Allele position around which to center the sliding windows.')
    parser.add_argument('--window_symmetric', type=utils.str2bool, default=False, help='Whether to enforce symmetric windowing.')
    parser.add_argument('--strand_reduction', type=str, choices=('mean', 'sum', 'max', 'min', 'abs_max', 'abs_min', 'gather', 'pick_forward', 'pick_reverse'), default='mean', help='Specify reduction over strands.')
    parser.add_argument('--window_reduction', type=str, choices=('mean', 'sum', 'max', 'min', 'abs_max', 'abs_min', 'gather'), default='mean', help='Specify reduction over testing windows.')
    ...
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size during sequence extraction from FASTA.')
    ...
    parser.add_argument('--extract_original_sequences', type=utils.str2bool, default=False, help='Extract sequences using original OneHotSlicer method for comparison.')
    parser.add_argument('--original_sequence_output_file', type=str, help='File path for original sequence output. If not specified, uses output path with _original_sequences.tsv suffix.')
    parser.add_argument('--use_simple_padding', type=utils.str2bool, default=False, help='Use simple MPRA padding for indels instead of genomic shifting.')
    parser.add_argument('--extract_center_sequences', type=utils.str2bool, default=False, help="Extract the 200bp center sequences from the new indel handling method for validation.")
    parser.add_argument('--center_sequence_output_file', type=str, help="File path for the new method's center sequence output. If not specified, uses output path with _center_sequences.tsv suffix.")
    parser.add_argument('--extract_flank_sequences', type=utils.str2bool, default=False, help="Extract the 200bp flank sequences from the new indel handling method for validation.")
    parser.add_argument('--flank_sequence_output_file', type=str, help="File path for the new method's flank sequence output. If not specified, uses output path with _flank_sequences.tsv suffix.")
    parser.add_argument('--extract_full_sequences', type=utils.str2bool, default=False, help="Extract the complete 600bp sequences from the new indel handling method for validation.")
    parser.add_argument('--full_sequence_output_file', type=str, help="File path for the new method's full sequence output. If not specified, uses output path with _full_sequences.tsv suffix.")
